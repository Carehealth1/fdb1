    ...
    if result["status"] == "success":
        ...
        with tab1:
            if isinstance(result.get("data"), dict):
                # Display Items if present (for search results)
                if "Items" in result["data"]:
                    st.write(f"Total Results: {result['data'].get('TotalResultCount', 0)}")
                    for item in result["data"]["Items"]:
                        st.write("---")
                        if "PrescribableDrugDesc" in item:
                            st.write(f"Drug: {item['PrescribableDrugDesc']}")
                        if "DispensableGenericDesc" in item:
                            st.write(f"Generic: {item['DispensableGenericDesc']}")
                        if "DefaultETCDesc" in item:
                            st.write(f"Category: {item['DefaultETCDesc']}")
    ...
    if api_option == "Drug Search":
        ...
        if st.button("Search") and search_term.strip():
            ...
            if result["status"] == "no_results":
                ...
                st.info("Example: Try searching for 'clopidogrel' as shown in the documentation")
            else:
        ...
        if st.button("Check Interactions") and drug_id:
            ...
            endpoint = f"PrescribableDrugs/{drug_id}/DrugInteractions"
            result = make_api_request(endpoint, client_id, client_secret, params)
            display_result(result)
        ...
        if st.button("Get Dosing Info") and drug_id:
            ...
            endpoint = f"PrescribableDrugs/{drug_id}/DoseRecords"
            result = make_api_request(endpoint, client_id, client_secret, params)
            display_result(result)
